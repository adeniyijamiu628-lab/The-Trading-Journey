...
// ✅ FIX 1: Move helper function OUTSIDE the component or inside, but OUTSIDE useEffect
const formatValue = (value) => {
  // Use standard check for null/undefined/0.
  if (value === null || value === undefined || value === 0) {
    return "";
  }
  // Convert number-like values to string for input fields
  return String(value);
};
...
useEffect(() => {
  if (isOpen && trade) {
    // ... (Your existing helper functions like formatValue, formatDateForInput)

    // Helper to extract YYYY-MM-DD from an ISO string
    const formatDateForInput = (isoString) =>
      isoString ? isoString.split("T")[0] : "";

    // 1. Format Dates
    // Use the snake_case DB field: entry_date
    const formattedEntryDate = formatDateForInput(trade.entry_date); 
    // Use the snake_case DB field: exit_date
    const formattedExitDate = formatDateForInput(trade.exit_date);

    // 2. Format Time (The Fix for tradeTime)
    // ✅ FIX 4: Extract time (HH:MM) from the entry_date timestamp
    const formattedTime = trade.entry_date
      ? trade.entry_date.split("T")[1]?.substring(0, 5) // Split by 'T', get the time part (HH:MM)
      : "";
    ...
    setFormData({
      ...
      tradeTime: formattedTime, // <-- Now correctly filled

      // Price and Risk Fields
      // Access snake_case DB field: entry_price
      entryPrice: formatValue(trade.entry_price),
      sl: formatValue(trade.sl), 
      tp: formatValue(trade.tp), 
      risk: formatValue(trade.risk) || "2.0", 

      strategy: trade.strategy ?? "",
      session: trade.session ?? "",
      
      // ✅ FIX 5: Access the lowercase DB field: beforeimage
      beforeimage: trade.beforeimage ?? "", 
      
      exitDate: formattedExitDate,
      // Access snake_case DB field: exit_price
      exitPrice: formatValue(trade.exit_price),
      // Access snake_case DB field: pnl_currency
      pnlCurrency: formatValue(trade.pnl_currency), 
      
      // ✅ FIX 6: Access the lowercase DB field: afterimage
