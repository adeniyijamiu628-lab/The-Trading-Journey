...
const loadJournalFromSupabase = async (uid, accountId) => {
  console.log("🔍 Loading trades from Supabase for:", { uid, accountId });
  ...
  if (error) {
    console.error("❌ Supabase trades load error:", error);
    return null;
  }
...
// ✅ Fixed: Prevent overwriting trades with empty arrays
useEffect(() => {
  if (!userId || !currentAccountId) return;
...
{isEditModalOpen && editingTrade && (
  <TradeEditModal
    ...
    onClose={() => {
      // ensure both flags are cleared so modal unmounts
      setIsEditModalOpen(false);
      setEditingTrade(null);
    }}
...
case "settings":
      ...
      <main className="flex-1 bg-gray-800 rounded-2xl shadow-lg border border-gray-700 p-8">
        ...
        {settingsView === "accountManager" && (
          <div>
            ...
            {/* ✅ Use the same AccountManager component from dashboard */}
  ...
  onAccountsChange={(updatedAccounts) => setAccounts(updatedAccounts)} // ✅ Added for real-time delete persistence
/>
