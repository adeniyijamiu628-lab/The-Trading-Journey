...
const normalizeTradeForDB = (t, userId, accountId) => {
  ...
  else status = rawStatus === "" ? "open" : rawStatus; // fallback: preserve if already canonical or default to open
  ...
  return {
    ...
    pair: t.pair ?? null,
    type: t.type ?? null,
    entry_date: t.entryDate ?? t.entry_date ?? null,
    ...
    status: status,
    ratio: t.ratio ?? null,
    before_image: t.beforeImage ?? t.before_image ?? null,
    after_image: t.afterImage ?? t.after_image ?? null,
    exit_date: t.exitDate ?? t.exit_date ?? null,
...
const persistJournal = async (openTrades = tradesOpen, historyTrades = tradesHistory) => {
  try {
    ...
    const { data, error } = await supabase.from("trades").upsert(rows, { onConflict: ["id"] });

    if (error) {
      console.error("persistJournal error:", error);
      toast.error("Failed to persist trades.");
    } else {
...
// Adjust value per pip depending on account type
const getAdjustedVP = (pair, accountType) => {
  const base = vpValues?.[pair] || 10;
  if (!accountType) return base;
  const t = accountType.toLowerCase();
  if (t === "mini") return base / 10;
  if (t === "micro" || t === "cent") return base / 100;
  return base;
};
...
// Save updated trade back to Supabase
const handleSaveEditedTrade = async (updatedTrade) => {
  try {
    ...
    // Optional: Refresh closed trades after saving
    fetchClosedTrades?.();
  } catch (err) {
    console.error("❌ Failed to update trade:", err.message);
  }
};
  ...
  <p className="text-sm text-gray-300 mt-2">
    Session: {getSessionForTime(tradeTime)}
      ...
      <div className="mt-4 p-4 rounded-xl bg-gray-700 border border-gray-600 space-y-2 text-sm text-gray-300">
        ...
        <p><span className="font-semibold">Est. Risk:</span> {coloredBySign(liveStopLossCurrency)}</p>
        <p><span className="font-semibold">Est. Profit:</span> {coloredBySign(liveTakeProfitCurrency)}</p>
    ...
    <AnimatePresence>
      {sortedFilteredHistory.map((t) => (
        ...
        >
          ...
          <td className="px-4 py-3">{t.exitDate ?? ""}</td>
          <td className="px-4 py-3">
            <div className="flex gap-2">
              <button className="px-3 py-1 rounded-full bg-gray-700 text-white" onClick={() => handleEditTrade(trade)}>Edit</button>
              <button className="px-3 py-1 rounded-full bg-red-600 text-white" onClick={() => handleDeleteTrade(t.id)}>Delete</button>
            </div>
          </td>
        </motion.tr>
      ))}
